...
inline static void perspectiveTranform(const std::vector<cv::Point>& originalPoints,
	...
	for (unsigned int i = 0; i < originalPoints.size(); i++) {
        ...
        float xet = transformMatrix.at<float>(0, 0) * (float)x +
                transformMatrix.at<float>(0, 1) * (float)y +
				transformMatrix.at<float>(0, 2);
        float xdt = transformMatrix.at<float>(2, 0) * (float)x +
                transformMatrix.at<float>(2, 1) * (float)y +
				transformMatrix.at<float>(2, 2);
        float yet = transformMatrix.at<float>(1, 0) * (float)x +
                transformMatrix.at<float>(1, 1) * (float)y +
